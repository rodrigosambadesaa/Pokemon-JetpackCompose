...
`ConnectivityAndInternetAccess.isConnected(...)`. La implementación actual considera también
si una VPN conserva una red subyacente utilizable; si no hay una red efectiva, la petición se
pospone y la interfaz muestra el estado offline. Cuando existe conectividad, se ejecuta la
petición real de PokéAPI con sus propios timeouts y manejo de errores, sin sondeos activos
redundantes ni enumeración de hosts.
...
La UI conserva un `NetworkObserver` pasivo durante su lifecycle y reacciona a cambios de red,
incluidos VPN, underlying network, pérdida de validación y portal cautivo. `NET_CAPABILITY_VALIDATED`
y `CAPTIVE_PORTAL` se mantienen como señales independientes; el diagnóstico general solo se usa
como fallback tras un fallo de transporte o cuando el usuario lo solicita.
